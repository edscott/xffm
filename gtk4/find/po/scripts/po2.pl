...
use File::Basename;
# 
# Run from the directory where cooked files will be generated.
# This directory should be one below the level where the raw
# files directory is located.
#
...
#$workdir = "/raid/home/SVN/xffm/CURRENT/full-po.5.0";
#$rodentdir = "/home/SVN/xffm/CURRENT/rodent";
$workdir = "/usb/full-po.5.0";
$rodentdir = "/usb/rodent";


`mkdir $workdir/source_po-raw`;
`mkdir $workdir/source_po-raw/xffm`;
`rm $workdir/source_po-raw/xffm/rodent`;
`ln -s $rodentdir/po $workdir/source_po-raw/xffm/rodent`;
$dry_run=1;

foreach $group (@groups) {

$contribdir= "$workdir/source_po-raw/$group";
$_=`ls $contribdir`;
@dirs=split;
$buffer="";
print "contrib---------  $contribdir\n";
foreach $dir (@dirs) {
    print "---------  $contribdir/$dir/\n"; #next;
   $_=`ls $contribdir/$dir/*po`;#next;
#    $_=`ls $contribdir/$dir/zh_TW.po`;#next;
    @files=split;
    foreach $file (@files){
