...
public:
    ...
    static void
    init(void){
     /* auto iconDir = g_strconcat(g_get_user_cache_dir(), G_DIR_SEPARATOR_S, "icons", NULL);
      if (!g_file_test(iconDir, G_FILE_TEST_IS_DIR)) {
        if (g_mkdir_with_parents(iconDir, 0700) < 0){
          DBG("Error::initCairoSvg():%s\n", strerror(errno));
          exit(1);
        }
      }*/
    }
    ...
    */
    ...
    add_label_pixbuf(cairo_t *pixbuf_context, GdkPixbuf *pixbuf, const gchar *icon_text){
        ...
        PangoContext *context = gtk_widget_get_pango_context (MainWidget);
    ...
    static GdkPixbuf *
    insertPixbuf(GdkPixbuf *base_pixbuf, GdkPixbuf *thumb, const gchar *where){
        cairo_t   *pixbuf_context = pixbuf_cairo_create(base_pixbuf);
        gdk_cairo_set_source_pixbuf(pixbuf_context, base_pixbuf,0,0);
        cairo_paint_with_alpha(pixbuf_context, 1.0);
        insert_pixbuf_tag (pixbuf_context, thumb, base_pixbuf, where, "1.0", "255");
        auto pixbuf = pixbuf_cairo_destroy(pixbuf_context, base_pixbuf);
        return pixbuf;
    }
