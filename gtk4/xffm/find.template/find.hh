...
//          gtk_notebook_set_action_widget(notebook_, GTK_WIDGET(findButton_), GTK_PACK_END);
  ...
  // FIXME: use textview completion.
        ...
        void mkButtonBox(void){
            ...
            g_object_set_data(G_OBJECT(mainBox_), "fgrCancelButton", cancelButton);
            g_object_set_data(G_OBJECT(cancelButton), "mainBox", mainBox_);
            gtk_widget_set_sensitive(GTK_WIDGET(cancelButton), FALSE);

            auto fgrClearButton =  boxButton(EMBLEM_CLEAR,(void *) FindSignals<Type>::onClearButton);


            g_object_set_data(G_OBJECT(mainBox_), "fgrClearButton", fgrClearButton);
            g_object_set_data(G_OBJECT(fgrClearButton), "mainBox", mainBox_);

            GtkBox *fgrEditButton = NULL;
            ...
            if (editor && strlen(editor)){
                ...
                if (editor_path){
                    ...
                    fgrEditButton = boxButton(icon_id, (void *)FindSignals<Type>::onEditButton);
                    g_free(icon_id);
                    g_object_set_data(G_OBJECT(mainBox_), "fgrEditButton", fgrEditButton);
                    g_object_set_data(G_OBJECT(fgrEditButton), "mainBox", mainBox_);
                    g_free(editor_path);
                    gtk_widget_set_sensitive(GTK_WIDGET(fgrEditButton), FALSE);
                } 
            } else {
                TRACE("getEditor() = \"%s\"\n", editor);
            }
          
            gtk_box_append(hbuttonbox2, GTK_WIDGET(fgrClearButton));
            if (fgrEditButton) gtk_box_append(hbuttonbox2, GTK_WIDGET(fgrEditButton));
...
//        GtkTextView *diagnostics = GTK_TEXT_VIEW(g_object_get_data(G_OBJECT(mainBox), "diagnostics"));
        ...
        // In dialog mode, only one fgr windows at a time...
        // This only allows a single "fgrCancelButton" in entire app:
        // Dialog is already created by Find class template.
        GtkWidget *cancel = GTK_WIDGET(g_object_get_data(G_OBJECT(mainBox), "fgrCancelButton"));
        //gtk_widget_show(cancel);
        gtk_widget_set_sensitive(cancel, TRUE);

#if 10
        /* get the parameters set by the user... *****/
        auto fgrData = (FgrData *) new FgrData(mainBox);
        fgrData->set_arguments(path);

        // run the fgr command.
        auto f = new fgr<FgrData>(fgrData);
#else
        // This is in standalone mode. Cleanup para huevones.
  ...
  public:
    ...
    Cleanup (void *data) {
       ...
       GtkWidget *fgrEditButton = GTK_WIDGET(g_object_get_data(G_OBJECT(mainBox), "fgrEditButton"));
       if (g_slist_length(lastFind)){
            ...
            gtk_widget_set_sensitive(GTK_WIDGET(fgrEditButton), TRUE);
       } else {
            gtk_widget_set_sensitive(GTK_WIDGET(fgrEditButton), FALSE);
       }
       if (Data->done) {
    ...
    cancel_all(void * dialog){
        ...
        GtkWidget *cancel = GTK_WIDGET(g_object_get_data(G_OBJECT(dialog), "fgrCancelButton"));
        gtk_widget_set_sensitive(cancel, FALSE);
    }
