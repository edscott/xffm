...
public:
        ...
        gint size)
    {
        ...
        if (textType){
             GdkPaintable *paintable = textPreview (filePath, PREVIEW_IMAGE_SIZE); 
             TRACE("%s texttype paintable=%p\n", filePath, paintable);
             return paintable;
        }

        GdkPaintable *previewPixbuf = NULL;

        // Properties modules comes here for image previews too.
        // We short circuit for images.
        // XXX circular dependency here
        /*if (strstr(mimetype, "image")){
            return Texture::getImageAtSize(filePath, PREVIEW_IMAGE_SIZE, mimetype, st_p);
        }*/
        ...
        if (size != PREVIEW_IMAGE_SIZE){
            ...
            TRACE("previewDefault(%s)...Texture::getImageAtSize %d\n", filePath, size);

            previewPixbuf = 
                Texture::getImageAtSize(filePath, size, mimetype, st_p);
            // We get the preview image path:
            auto previewPath = 
                Texture::get_thumbnail_path (filePath, size);
            ...
            if (g_file_test(previewPath, G_FILE_TEST_EXISTS)) {
                paintable = Texture::buildImagePixbuf(previewPath, size);
                // Since this is loading a png image, label is wrong.
                // Overwrite label:
             /*   auto label = " pdf";
                if (strstr(mimetype, "postscript")) label = "  ps";
                Texture::insertPixbufLabel(paintable, label);*/

            }
            g_free(previewPath);
            TRACE("previewDefault(%s)... OK paintable=%p\n", filePath, paintable);

            return (paintable);
        } 

        // Now we proceed to generate a paintable at requested size...
        ...
        // From here we only create paintable at preview size PREVIEW_IMAGE_SIZE.
...
private:
    ...
    gsPreview (const gchar *path, gint pixels) {
        ...
        auto preview = Texture::get_thumbnail_path (path, PREVIEW_IMAGE_SIZE);
        //auto thumbnail = Texture::get_thumbnail_path (path, pixels);
    ...
    correctPixbufSize(const gchar *path, gint pixels){
        auto previewPath = Texture::get_thumbnail_path (path, PREVIEW_IMAGE_SIZE);
        if (!g_file_test(previewPath,G_FILE_TEST_EXISTS)){
        ...
        GError *error=NULL;
        auto previewPixbuf = gdk_paintable_new_from_file (previewPath, &error);
        if (error){
...
public:  
    ...
    zipPreview (const gchar *path, gint pixels) {
        ...
        gchar *text = Basic::pipeCommandFull(command);
        g_free(command);
...
private:
    ...
    readFileHead(const gchar *path) {
        auto encoding = Mime::encoding (path);
    ...
    text_preview_f (void *data){
        ...
        auto previewPixbuf = Texture::get_thumbnail_path (filePath, PREVIEW_IMAGE_SIZE);
        //auto thumbnail = Texture::get_thumbnail_path (filePath, pixels);
        ...
        GError *error = NULL;
        auto paintable = gdk_paintable_new_from_file (previewPixbuf, &error);
        if(error) {
...
private:
    static
    GdkPaintable *
    fixPixbufScale(GdkPaintable *in_paintable, gint size){
        if (!in_paintable || !GDK_IS_PIXBUF(in_paintable)) return NULL;
        GdkPaintable *out_paintable=NULL;
        gint height = gdk_paintable_get_height (in_paintable);
        gint width = gdk_paintable_get_width (in_paintable);
#if 10
        double scale = size;
        ...
        {
             out_paintable = gdk_paintable_scale_simple (in_paintable, scale * width, scale * height,
                 GDK_INTERP_HYPER);
            g_object_ref(out_paintable);
            g_object_unref(in_paintable);
            return out_paintable;
       }
#else
        ...
        {
            out_paintable = gdk_paintable_scale_simple (in_paintable, 5*size/7,size,
                 GDK_INTERP_HYPER);
            g_object_ref(out_paintable);
            g_object_unref(in_paintable);
            return out_paintable;
