#include "properties_c.hpp"

/*
 * Edscott Wilson Garcia Copyright 2001-2016
 *
...
properties_c::properties_c(void){
    selection_list = NULL;
}
...
void
properties_c::clear_selection_list(void){
    GList *l = selection_list;
    for (;l && l->data; l=l->next) g_free(l->data);
    g_list_free(selection_list);
    selection_list = NULL;
}

void
properties_c::do_prop(const gchar *file_path){
 
    properties_t *properties_p = (properties_t *)setup_properties_p(file_path);
  /*  if(properties_p->select_list==NULL){
    ...
    */
    gtk_p->context_function(dlg_prop,  (void *)properties_p);
...
void
properties_c::do_prop(GList *file_list){
    //FIXME
    clear_selection_list();
    ...
    for (;l && l->data; l=l->next){
        p_path = (gchar *)(l->data);
        selection_list = g_list_prepend(selection_list, g_strdup(p_path));
        // update stat info
        /*
	    if (stat((p_path, &p_st) < 0) {
...
void *
properties_c::setup(const gchar *file_path){
	NOOP("selection list count ==1\n");
        properties_p->file_path = g_strdup(file_path);
	...
	stat(file_path, &properties_p->st);
    return (void *)properties_p;
}


#if 0
void
properties_c::setup(const gchar *widgets_t *widgets_p){
