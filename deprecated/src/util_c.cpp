...
gint
util_c::length_equal_string(const gchar *a, const gchar *b){
    int length=0;
    int i;
...
gchar *
util_c::get_tilde_dir(const gchar *token){
    struct passwd *pw;
...
gint
util_c::compose_key (gint key, gint dead_key) {
    switch (dead_key) {
...
gchar *
util_c::valid_utf_pathstring (const gchar * string) {
...
gchar *
util_c::recursive_utf_string (const gchar * path) {
    gchar *dir,
     *base,
     *valid,
     *utf_base,
...
void
util_c::set_store_data_from_list(GtkListStore *list_store, GSList **list){
    GtkTreeIter iter;
...
/*
gchar *
util_c::wrap_utf_string(const gchar *data, gint length){
    gchar *u;
...
void *
util_c::context_function(void * (*function)(gpointer), void * function_data){
...
gchar *
util_c::utf_string (const gchar * t) {
    ...
    for(p = t; *p; p++) {
        ...
        if(outbuf_len < 0) {
            //DBG ("util_c::utf_string: unichar=%d char =%c outbuf_len=%d\n", gu, p[0], outbuf_len);
        }
...
gchar *
util_c::compact_line(const gchar *line){
...
GList *
util_c::find_in_string_list(GList *list, const gchar *string){
...
gboolean
util_c::program_in_path(const gchar *program){
...
gchar *
util_c::esc_string (const gchar * string) {
    gint i, j, k;
    const gchar *charset = "\\\"\' ()|<>";
...
const gchar * 
util_c::term_exec_option(const gchar *terminal) {
