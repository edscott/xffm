...
public:
    static void concat(char **log, const char *text){
      char *g = g_strconcat(*log, text, NULL);
      g_free(*log);
      *log = g;
      return;
    }
            ...
            GdkDragAction action)
    {
        ...
        if (number > 1) action = queryAction(view, tgt, files);
        ...
        g_free(more);
        auto result = Gio<Type>::executeURL(files, tgt, mode);
